...
export function buildCypherSelection({
  ...
  if(innerSchemaType && innerSchemaType.astNode && innerSchemaType.astNode.kind === "InterfaceTypeDefinition") {
      ...
      const commonSelections = headSelection.selectionSet.selections.filter(item => item.kind === "Field");
      ...
      const fragmentMap = fragments.map(fragment => {
        ...
        // merge selections
        const selections = [...commonSelections, ...fragment.selectionSet.selections];
        // get the graphqlObjectType for our fragment
        const schemaType = resolveInfo.schema._implementations[interfaceName].find(intfc => intfc.name === implementationName);

        // generate subselections
        const subSelections = extractSelections(selections, resolveInfo.fragments);
                  ...
                  !isArrayType(fieldType) ? 'head(' : ''
                    ...
                    fragmentMap.map((fragment) => {
                      ...
                      return `[(${variableName})${
                          ...
                          fragment.name
                          }${queryParams}) | ${varName} {${fragment.subSelection[0]}}]`
                    }).join(' + ')
                  }${
      ...
      return [selection[0], { ...selection[1], ...fragmentMap.map(fragment => fragment.subSelection[1]) }];
  }
